...
<head>
    ...
    <style>
        ...
        /* Khung chọn ngôn ngữ ở góc trên bên trái (cạnh nút menu hoặc thay thế) */
        .lang-selector-box { position: absolute; top: 15px; left: 60px; z-index: 10; background: #1e1e1e; border: 1px solid #333; border-radius: 8px; padding: 4px 8px; display: flex; align-items: center; gap: 5px; }
        .lang-selector-box select { background: #121212; color: #fff; border: 1px solid #444; border-radius: 5px; padding: 4px 6px; font-size: 12px; outline: none; cursor: pointer; }
        ...
        /* Tin nhắn kèm thời gian góc dưới kiểu app nhắn tin */
        ...
        .msg-time { position: absolute; bottom: 4px; right: 10px; font-size: 9px; opacity: 0.7; font-family: monospace; }
...
<body>
    ...
    <!-- Hộp chọn gói ngôn ngữ theo yêu cầu -->
    ...
    <div class="sidebar" id="sidebar">
        <div class="sidebar-header">
            <h3 id="txtHistoryTitle">Lịch sử trò chuyện 🌸</h3>
            <button class="btn-close-sidebar" onclick="toggleSidebar()">✕</button>
        </div>
        <button class="btn-new-chat" id="btnNewChat" onclick="startNewChat()"><span>➕</span> Cuộc trò chuyện mới</button>
        <button class="btn-clear-all" id="btnClearAll" onclick="clearAllHistory()">🗑️ Xóa toàn bộ lịch sử</button>
        
        <div class="theme-section">
            <div class="theme-title" id="txtThemeTitle">🎨 Màu giao diện:</div>
    ...
    <div class="chat-container">
        ...
        <div class="input-box">
            ...
            <button class="btn-send" id="btnSend" onclick="sendMessage()">Gửi</button>
        </div>
    </div>
    ...
    <script>
        ...
        let currentLang = localStorage.getItem('mina_lang') || 'vi';

        const i18n = {
            vi: {
                welcome: "Hellu đại vương! Đồng hồ và hệ thống đa ngôn ngữ đã sẵn sàng ❤️",
                historyTitle: "Lịch sử trò chuyện 🌸",
                newChat: "Cuộc trò chuyện mới",
                clearAll: "Xóa toàn bộ lịch sử",
                themeTitle: "🎨 Màu giao diện:",
                placeholder: "Nhắn gì đi bé iu❤️...",
                send: "Gửi",
                promptLang: "Hãy trả lời bằng tiếng Việt."
            },
            ja: {
                welcome: "こんにちは、大王様！時計と多言語システムが準備完了しましたよ ❤️",
                historyTitle: "チャット履歴 🌸",
                newChat: "新しいチャット",
                clearAll: "すべての履歴を削除",
                themeTitle: "🎨 テーマカラー:",
                placeholder: "何かメッセージを送ってね❤️...",
                send: "送信",
                promptLang: "日本語で返事してください。"
            },
            zh: {
                welcome: "你好，大王！时钟和多语言系统已经准备好了 ❤️",
                historyTitle: "聊天记录 🌸",
                newChat: "新对话",
                clearAll: "清空所有记录",
                themeTitle: "🎨 主题颜色:",
                placeholder: "发条消息吧，亲爱的❤️...",
                send: "发送",
                promptLang: "请用中文回答。"
            },
            ko: {
                welcome: "안녕하세요 대왕님! 시계와 다국어 시스템이 준비되었습니다 ❤️",
                historyTitle: "채팅 기록 🌸",
                newChat: "새 대화",
                clearAll: "모든 기록 삭제",
                themeTitle: "🎨 테마 색상:",
                placeholder: "메시지를 남겨주세요❤️...",
                send: "보내기",
                promptLang: "한국어로 답변해 주세요."
            },
            en: {
                welcome: "Hello my lord! The clock and multi-language system are ready ❤️",
                historyTitle: "Chat History 🌸",
                newChat: "New Chat",
                clearAll: "Clear All History",
                themeTitle: "🎨 Theme Color:",
                placeholder: "Type something here, my love❤️...",
                send: "Send",
                promptLang: "Please reply in English."
            }
        };

        function changeLanguage() {
            currentLang = document.getElementById('langSelect').value;
            localStorage.setItem('mina_lang', currentLang);
            updateUIStrings();
        }
        ...
        function updateUIStrings() {
            ...
            document.getElementById('btnNewChat').innerHTML = `<span>➕</span> ${t.newChat}`;
            document.getElementById('btnClearAll').innerText = `🗑️ ${t.clearAll}`;
            document.getElementById('txtThemeTitle').innerText = t.themeTitle;
            document.getElementById('userInput').placeholder = t.placeholder;
            document.getElementById('btnSend').innerText = t.send;
        }

        // Lấy thời gian thực tương ứng với Quốc gia/Ngôn ngữ được chọn
        function getFormattedTime(lang) {
            let localeMap = {
                vi: 'vi-VN',
                ja: 'ja-JP',
                zh: 'zh-CN',
        ...
        function loadChat(chatId) {
            ...
            if (hist.length === 0) {
                const timeStr = getFormattedTime(currentLang);
                const t = i18n[currentLang] || i18n.vi;
                chatBox.innerHTML = `<div class="message ai-message">${t.welcome}<span class="msg-time">${timeStr}</span></div>`;
            } else {
                hist.forEach(item => {
                    ...
                    displayHtml += `<span class="msg-time">${item.time || getFormattedTime(currentLang)}</span>`;
        ...
        function appendMessage(content, sender, timeStr, isHtml = false) {
            const chatBox = document.getElementById('chatBox'), msgDiv = document.createElement('div');
            msgDiv.className = `message ${sender === 'user' ? 'user-message' : 'ai-message'}`;
            let fullContent = content + `<span class="msg-time">${timeStr}</span>`;
            if (isHtml) { msgDiv.innerHTML = fullContent; } else { msgDiv.textContent = fullContent; }
            chatBox.appendChild(msgDiv);
            chatBox.scrollTop = chatBox.scrollHeight;
            return msgDiv;
        }
        ...
        async function sendMessage() {
            ...
            appendMessage(userDisplayContent, 'user', timeStr, true);
            ...
            try {
                ...
                const response = await fetch('/chat', {
                    ...
                    body: JSON.stringify({ message: text, image: sentImg, api_key: apiKey, history: cleanHistory, lang: currentLang, client_time: timeStr })
                });
                const data = await response.json();
                typingDiv.remove();
                
                const replyTime = getFormattedTime(currentLang);
                appendMessage(data.reply, 'ai', replyTime, true);
                
                chats[currentChatId].history.push({ role: 'model', content: data.reply, hasImage: false, time: replyTime });
                saveChats();
            } catch (err) {
                typingDiv.remove();
                const errTime = getFormattedTime(currentLang);
                appendMessage('[Lỗi kết nối server!]', 'ai', errTime, false);
            }
        }
    </script>
</body>
...
def chat():
    ...
    target_lang_instruction = lang_instructions.get(client_lang, lang_instructions["vi"])
    ...
    payload = {
        "system_instruction": {
            "parts": [{
                "text": f"Mày tên là Mina, là một AI VTuber. Mày xưng là Mina, nói chuyện nhí nhảnh, thân thiện. Thiết kế ra mày là Shimizu Haruki - Yamada Takahashi (học sinh cấp hai). Thời gian thực tế hiện tại của người dùng là: {client_time}. {target_lang_instruction}"
            }]
        },
