...
<head>
    ...
    <style>
        ...
        .btn-menu { position: absolute; top: 15px; left: 15px; background: #1e1e1e; color: #ff4757; border: 1px solid #333; padding: 8px 12px; border-radius: 8px; cursor: pointer; z-index: 10; transition: transform 0.2s, background 0.2s; }
        ...
        .avatar-box { width: 35px; height: 35px; border-radius: 50%; border: 2px solid #ff4757; overflow: hidden; background-color: #222; cursor: pointer; flex-shrink: 0; transition: transform 0.3s; animation: pulseGlow 2s infinite; }
        ...
        .btn-new-chat { background: #252525; color: white; border: 1px solid #444; padding: 10px; border-radius: 8px; cursor: pointer; text-align: left; font-size: 14px; margin-bottom: 10px; display: flex; align-items: center; gap: 8px; transition: background 0.2s; }
        .btn-new-chat:hover { background: #333; }
        .btn-clear-all { background: #2a1518; color: #ff4757; border: 1px solid #442226; padding: 8px; border-radius: 8px; cursor: pointer; text-align: center; font-size: 13px; margin-bottom: 15px; font-weight: bold; transition: background 0.2s; }
        ...
        .btn-action { background-color: #ff4757; color: white; border: none; padding: 8px 12px; border-radius: 5px; cursor: pointer; font-weight: bold; font-size: 13px; transition: background 0.2s; }
        ...
        .user-message { background: #ff4757; color: white; align-self: flex-end; box-shadow: 0 3px 10px rgba(255, 71, 87, 0.25); }
        .ai-message { background: #333; color: #ff6b81; align-self: flex-start; border: 1px solid #444; box-shadow: 0 3px 10px rgba(0, 0, 0, 0.2); }
        ...
        .typing-dot { width: 7px; height: 7px; background: #ff6b81; border-radius: 50%; animation: typingBounce 1.4s infinite ease-in-out both; }
        ...
        .input-box input[type="text"]:focus { border-color: #ff4757; }
        ...
        .btn-send { background: #ff4757; color: white; border: none; padding: 10px 16px; border-radius: 6px; cursor: pointer; font-weight: bold; transition: transform 0.1s, background 0.2s; }
...
<body>
    ...
    <script>
        ...
        function loadChat(chatId) {
            ...
            if (hist.length === 0) {
                ...
                hist.forEach(item => {
                    ...
                    // Hiển thị lại ảnh nhẹ nhàng nếu có lưu cờ
        ...
        async function sendMessage() {
            ...
            // Lưu lịch sử gọn gàng, tách biệt ảnh để không làm phình to token quota
            ...
            try {
                // Chỉ lấy 10 tin nhắn gần nhất và không gửi lại chuỗi Base64 nặng nề vào lịch sử cũ
...
def chat():
    ...
    for item in client_history:
        ...
        # Nếu phần tử cuối cùng có chứa ảnh mới gửi, đính kèm định dạng chuẩn cho Gemini đọc ảnh
    ...
    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={user_api_key}"
    payload = {
