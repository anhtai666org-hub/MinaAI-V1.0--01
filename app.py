...
<body>
    ...
    <script>
        ...
        async function sendMessage() {
            ...
            formData.append('history', JSON.stringify(chats[currentChatId].history.slice(-10, -1))); // Lấy lịch sử cũ trước đó
            ...
            try {
                ...
                chats[currentChatId].history.push({ role: 'model', content: data.reply, imageSrc: null });
                saveChats();
            } catch (err) {
                typingDiv.remove();
...
def chat():
    ...
    # Thêm tin nhắn mới nhất của user kèm ảnh (nếu có)
